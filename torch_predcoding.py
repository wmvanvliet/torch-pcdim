...
class MiddleLayer(nn.Module):
    ...
    def __init__(
        ...
        if bu_weights is None:
            bu_weights = torch.clamp(torch.randn(n_in, n_units) * 0.1, 0)
        if td_weights is None:
            td_weights = torch.clamp(torch.randn(n_units, n_in) * 0.1, 0)
    ...
    def train_weights(self, bu_err, lr=0.01):
        ...
        delta = ye - self.state.sum(axis=0, keepdims=True) * (
            self.bu_weights.sum(axis=0, keepdims=True) + Vw
        )
        change = torch.abs(delta).max(axis=0, keepdims=True)[0]
        bu_weights = self.bu_weights + lr * delta * self.bu_weights.max(
            axis=0, keepdims=True
        )[0] / torch.maximum(self.one, change)
        bu_weights = torch.clamp(bu_weights, 0)
        self.bu_weights.set_(bu_weights)

        delta = ye.T - self.state.sum(axis=0, keepdims=True).T * (
            self.td_weights.max(axis=1, keepdims=True)[0] + self.td_weights
        )
        change = torch.abs(delta).max(axis=1, keepdims=True)[0]
        td_weights = self.td_weights + lr * delta * self.td_weights.max(
...
class InputLayer(nn.Module):
    ...
    def __init__(self, n_units, batch_size=1, bu_weights=None, eps_1=0.01, eps_2=0.01):
        super().__init__()
        self.n_units = n_units
        self.batch_size = batch_size
    ...
    def forward(self, x=None):
        ...
        self.td_err = self.reconstruction / torch.maximum(self.eps_1, self.state)
        return self.state / torch.maximum(self.eps_2, self.reconstruction)

...
trainable_weights = get_weights(data_path, train_V_lex_orth = True)
training_batch_size = 1579 # set how many inputs to train on. I initially used 10 for debugging purposes
m = PCModel(trainable_weights, batch_size=training_batch_size)
...
for n in tqdm(range(1,training_n_iterations+1), unit="step"):
    ...
    m(clamp_orth=training_batch, clamp_ctx=training_batch,train_weights = train_weights)
    train_weights = False
...
# input_batch = lex[:512]
# load_weights = get_weights(data_path, load_trained_V_lex_orth = True)
# trained_model = PCModel(load_weights, batch_size=512)
# for n in tqdm(range(n_iterations), unit="step"):
#     trained_model(clamp_orth=input_batch)
